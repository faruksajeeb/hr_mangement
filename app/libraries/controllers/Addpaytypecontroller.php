...
class Addpaytypecontroller {
    public function __construct()
    {
        $this->CI =& get_instance();
    }
    function add_paytype_action($post_data=array()){
        //$ci=& get_instance();
        $this->CI->load->database();
        $this->CI->load->model('taxonomy');
        $msg = "";
        $vid=9;
        $paytype=$post_data['paytype'];
        $description=$post_data['description'];
        $content_id=$post_data['content_id'];
        $check_duplicate=$this->CI->taxonomy->getTaxonomybyname($vid, $paytype);
            $t_id=$this->CI->taxonomy->getLasttaxonomyId();
            $toadd_tid=$t_id+1;
        if($paytype && !$check_duplicate){
            if($content_id){
