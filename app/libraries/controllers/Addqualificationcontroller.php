...
class Addqualificationcontroller {
    public function __construct()
    {
        $this->CI =& get_instance();
    }
    function add_qualification_action($post_data=array()){
        //$ci=& get_instance();
        ...
        $check_duplicate=$this->CI->taxonomy->getTaxonomybyname($vid, $qualification);
            $t_id=$this->CI->taxonomy->getLasttaxonomyId();
            $toadd_tid=$t_id+1;
        if($qualification && !$check_duplicate){
            if($content_id){
