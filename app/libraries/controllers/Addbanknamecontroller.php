...
class Addbanknamecontroller {
    public function __construct()
    {
        $this->CI =& get_instance();
    }
    function add_bankname_action($post_data=array()){
        //$ci=& get_instance();
        ...
        $check_duplicate=$this->CI->taxonomy->getTaxonomybyname($vid, $bankname);
            $t_id=$this->CI->taxonomy->getLasttaxonomyId();
            $toadd_tid=$t_id+1;
        if($bankname && !$check_duplicate){
            if($content_id){
