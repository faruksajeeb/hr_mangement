...
class Add_marital_status_controller {
    public function __construct()
    {
        $this->CI =& get_instance();
    }
    function add_marital_status_action($post_data=array()){
        //$ci=& get_instance();
        ...
        $check_duplicate=$this->CI->taxonomy->getTaxonomybyname($vid, $marital_status);
            $t_id=$this->CI->taxonomy->getLasttaxonomyId();
            $toadd_tid=$t_id+1;
        if($marital_status && !$check_duplicate){
            if($content_id){
