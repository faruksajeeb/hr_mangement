...
$payroll_report = $this->users_model->getuserwisepermission("payroll_report", $user_id);
$salary_report = $this->users_model->getuserwisepermission("salary_report", $user_id);
$view_loan_report = $this->users_model->getuserwisepermission("view_loan_report", $user_id);
$salary_certificate = $this->users_model->getuserwisepermission("salary_certificate", $user_id);
$tax_calculator = $this->users_model->getuserwisepermission("tax_calculator", $user_id);
...
if ($user_type == 8) { // Store area
    ...
    ?>
            ...
            <?php }
if ($user_type == 1 || $view_monthly_attendance['status'] == 1 ||
    ...
    $view_daily_leave_reports['status'] == 1 ||
    $view_single_leave_reports['status'] == 1 ||
    $view_leave_summery_reports['status'] == 1 ||
    $view_attendance_informed_reports['status'] == 1 ||
    $view_attendance_log_reports['status'] == 1 ||
    $view_salary_history['status'] == 1 ||
    $view_salary_history_multiple['status'] == 1 ||
    $view_job_history['status'] == 1 ||
    $view_resume['status'] == 1 ||
    $view_holiday_reports['status'] == 1 ||
    $tax_calculator['status'] == 1 ||
    $pf_statement['status'] == 1
              ...
              <ul>
                ...
                <?php }?>
                <?php if ($user_type == 1 || $payroll_report['status'] == 1): ?>
                ...
                <?php if ($user_type == 1 || $salary_report['status']) :?>
                  <li><a href="#">Salary <span class="pull-right glyphicon glyphicon-menu-right"></span></a>
                    <ul>
                      ...
                      <?php } if ($user_type == 1 || $view_salary_history_multiple['status'] == 1) {?>
