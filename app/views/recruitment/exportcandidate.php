...
submitHandler: function(form) {
    ...
    if(division_code == emp_code){
		$("#emp_id").css({"border":"1px solid red"});
    }else if(division_code != emp_code_divison_code){
		$("#emp_id").css({"border":"1px solid red"});
...
$( "#emp_division" ).change(function(e) {
	...
	var division_code = $('option:selected', this).attr('data-id');
	$("#emp_id").val(division_code); 
	$("#emp_id").focus(); 
	var base_url='<?php echo base_url();?>';
	var postData = {
...
$('body').on('keydown', '.datepicker', function(e) {
  ...
  // inputs.eq( inputs.index(this)+ 1 ).focus();
     ...
     $("#emp_basic_salary").keyup(function(){
     	...
     	var houserent_allowance = (basic_salary * 50/100); 
     	var medical_allowance = (basic_salary * 20/100); 
     	var convayance_allowance = (basic_salary * 20/100); 
     	var provident_allowance = (basic_salary * 10/100); 
     	var transport_allowance = (basic_salary * 15/100); 
     	var others_allowance = (basic_salary * 10/100); 
     	$('#emp_house_rent').val(houserent_allowance);
     	$('#emp_medical_allowance').val(medical_allowance);
     	$('#emp_conveyance').val(convayance_allowance);
     	$('#emp_provident_fund_allowance').val(provident_allowance);
     	$('#emp_transport_allowance').val(transport_allowance);
     	$('#emp_other_allowance').val(others_allowance);
     	// $('#emp_conveyance').val(convayance_allowance);
     });  
...
$("#emp_salary_increment_amount").keyup(function(){
	var total_salary_increment=$(this).val();
	// alert(total_salary);
	// Math.round(65.98);
	var basic_salary=Math.round(total_salary_increment/100*44.44445);
	var house_rent=Math.round(total_salary_increment/100*22.22223);
	var medical_allow=Math.round(total_salary_increment/100*8.88888889);
	...
	var transport_allow=Math.round(total_salary_increment/100*6.6666667);
	var without_other_allow=basic_salary+house_rent+medical_allow+conveyance_allow+provident_allow+transport_allow;
	var other_allow=Math.round(total_salary_increment-without_other_allow);
	var total_benifit=house_rent+medical_allow+conveyance_allow+provident_allow+transport_allow+other_allow;
	$('#emp_basic_salary').val(basic_salary);
	$('#emp_house_rent').val(house_rent);
	$('#emp_medical_allowance').val(medical_allow);
	$('#emp_conveyance').val(conveyance_allow);
	$('#emp_provident_fund_allowance').val(provident_allow);
	...
	if($('#emp_salary_increment').is(':checked')) {
		var total_gross_salary=parseInt(present_salary)+parseInt(total_benifit)+parseInt(basic_salary)-parseInt(total_deduction);
		var emp_increment_percentage= Math.round(total_salary_increment*100/present_salary); 
	}else if($('#emp_salary_update').is(':checked')){
		var total_gross_salary=parseInt(previous_salary)+parseInt(total_benifit)+parseInt(basic_salary)-parseInt(total_deduction);
		var emp_increment_percentage= parseInt(previous_salary)/parseInt(total_salary_increment*100); 
	}else{
		var total_gross_salary=parseInt(total_benifit)+parseInt(basic_salary)-parseInt(total_deduction); 
	}	
	$('#emp_gross_salary').val(total_gross_salary);
	var yearly_salary= total_gross_salary * 12;
	$('#emp_yearly_paid').val(yearly_salary);  
...
$("#deduction_area").keyup(function(){
    //   var total_loan_month=monthDiff(
    // new Date(dateParts[2], (dateParts[1] - 1), dateParts[0]), // November 4th, 2008
    // new Date(dateParts2[2], (dateParts2[1] - 1), dateParts2[0])  // March 12th, 2010
    // );	
var emp_provident_fund_deduction= $('#emp_provident_fund_deduction').val(); 
if(!emp_provident_fund_deduction) {
	var emp_provident_fund_deduction = 0;
	...
	var emp_total_salary= $('#emp_gross_salary').val(); 
	if(!emp_total_salary) {
		var emp_total_salary = 0;
	}
	var basic_salary= $('#emp_basic_salary').val(); 
	if(!basic_salary) {
		var basic_salary = 0;
	}
	var total_benifit= $('#emp_total_benifit').val(); 
	if(!total_benifit) {
		var total_benifit = 0;
	}							
	var total_deduction=parseInt(emp_provident_fund_deduction)+	parseInt(emp_other_deduction);	
	// var total_deduction_without_loan=parseInt(emp_provident_fund_deduction)+parseInt(emp_other_deduction) *12;	
	// var total_loan_deduction=emp_advance_loan*total_loan_month;
	// var total_yearly_deduction=total_deduction_without_loan+total_loan_deduction;
	if($('#emp_salary_increment').is(':checked')) {
		var total_gross_salary=parseInt(present_salary)+parseInt(total_benifit)+parseInt(basic_salary);
		var final_gross_salary=parseInt(total_gross_salary)- parseInt(total_deduction);
	}else if($('#emp_salary_update').is(':checked')){
		var total_gross_salary=parseInt(previous_salary)+parseInt(total_benifit)+parseInt(basic_salary);
		var final_gross_salary=parseInt(total_gross_salary)- parseInt(total_deduction);
	}else{
		var final_gross_salary=+parseInt(total_benifit)+parseInt(basic_salary)- parseInt(total_deduction); 
	}		
	// var emp_loan_deduction_from= $('#emp_loan_deduction_from').val(); 			
	// var emp_loan_deduction_to= $('#emp_loan_deduction_to').val(); 			
	// var dateParts = emp_loan_deduction_from.split("-");
	// var dateParts2 = emp_loan_deduction_to.split("-");
	$('#emp_total_deduction').val(total_deduction);  	
	$('#emp_gross_salary').val(final_gross_salary);  
	var yearly_final_salary=final_gross_salary*12;	
	$('#emp_yearly_paid').val(yearly_final_salary);  	
});
...
if($search_field_emp_data['emp_division']){
	$emp_division=$search_field_emp_data['emp_division'];
}
if($search_field_emp_data['emp_department']){
	$emp_department=$search_field_emp_data['emp_department'];
}
			...
			<div class="wrapper">
				<div class="row">
					<div class="col-md-12">
						...
						?>
						<form action="<?php echo base_url(); ?>recruitment/do_upload" method="post" class="myForm" id="myForm"  name="myForm" onSubmit="return validate();" enctype="multipart/form-data">
							<!-- <input type="hidden" id="content_id" name="content_id" value="<?php echo $content_id;?>"> -->
							 <input type="hidden" id="content_id_candidate" name="content_id_candidate" value="<?php echo $content_id;?>"> 
									...
									<div class="tabcontent" id="content_1">
										...
										<div class="row">
											<div class="col-md-6">	
												...
												<div class="row">
													<div class="col-md-4 bgcolor_D8D8D8">Grade</div>
													<div class="col-md-8">
												...
												<div class="row">
													...
													<div class="col-md-8">
														<input type="checkbox" name="emp_visa_selling" id="emp_visa_selling" <?php if($emp_visa_selling){ echo 'checked="checked"';} ?>/>        
													</div>
												</div>													
												<div class="row">
												...
												<div class="row">
													...
													<div class="col-md-10">
														...
														<select name="emp_weekly_holiday[]" id="emp_weekly_holiday" size="7" multiple>
															<option value="fri" <?php if($emp_holiday['fri_off']=='off'){print 'selected="selected"';}else if(!$emp_holiday){ print 'selected="selected"';} ?>>Friday</option>
										...
										<div class="total_leave_area">
											<div class="row">
												...
												<div class="row">
														...
														<div class="col-md-8">
															<input type="text" name="annual_leave_total" id="annual_leave_total"  onkeypress="return isNumber(event);" value="<?php if($emp_total_leave){ echo $emp_total_leave['total_days'];}else{ echo "12";} ?>"/>        
														</div>
													</div>													
												</div>
											</div>
...
<!-- 										<div class="row">
								...
								<div class="tabcontent" id="content_8">
									...
									<!-- <div class="col-md-2 increament heading_style text-right" style="<?php if($salary_count){ echo "padding:3px;"; }else{ echo "padding:12.5px;";} ?>"><?php if($salary_count){ ?>Update	<input type="checkbox" name="emp_salary_update" id="emp_salary_update" <?php if($emp_salary_update){ echo 'checked="checked"';} ?>/><?php } ?></div> -->
									...
									<div class="row">
										<div class="col-md-6">
											...
											<div class="row">
												...
												<div class="col-md-8">
													<input type="text" name="emp_festival_bonus" id="emp_festival_bonus" onkeypress="return isNumber(event);" value="<?php if($emp_festival_bonus){ echo $emp_festival_bonus;} ?>"/>        
												</div>
											</div>																																																										
										</div>
										<div class="col-md-6">
