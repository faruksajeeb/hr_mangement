...
class IncentiveController extends CI_Controller
{
    ...
    function create()
    {
        ...
        echo  $this->load->view('payroll/incentive/add_incentive', $data);
    }
    function save()
    {
        date_default_timezone_set('Asia/Dhaka');
        $data['title'] = "Save Incentive Bonus";
        ...
        if ($editId = $input['edit_id']) {
            $this->db->where('id', $editId)->update('tbl_incentive', $salaryData);
            $this->session->set_flashdata('message', "Incentive data updated successfully!");
        } else {
            $this->db->insert('tbl_incentive', $salaryData);
            $this->session->set_flashdata('message', "Incentive added successfully!");
        }
    ...
    function check_duplicate($emp_content_id)
    {
        ...
        $result = $this->db->get('tbl_incentive')->num_rows();
