...
class PfPaymentController extends CI_Controller
{
    ...
    function manage()
    {
        ...
        FROM tbl_pf_payments MA 
        LEFT JOIN search_field_emp SFE ON SFE.content_id=MA.content_id 
        ")->result();
        $this->load->view('payroll/pf_payment/manage_pf_payment', $data);
    }
    function create()
    {
        ...
        echo  $this->load->view('payroll/pf_payment/add_pf_payment', $data);
    }
    function save()
    {
        date_default_timezone_set('Asia/Dhaka');
        $data['title'] = "Save PF Payment";
        ...
        if ($editId = $input['edit_id']) {
            $this->db->where('id', $editId)->update('tbl_pf_payments', $salaryData);
            $this->session->set_flashdata('message', "PF data updated successfully!");
        } else {
            $this->db->insert('tbl_pf_payments', $salaryData);
            $this->session->set_flashdata('message', "PF added successfully!");
        }
    ...
    function check_duplicate($emp_content_id)
    {
        ...
        $result = $this->db->get('tbl_pf_payments')->num_rows();
