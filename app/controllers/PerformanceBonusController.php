...
class PerformanceBonusController extends CI_Controller
{
    ...
    function manage()
    {
        ...
        FROM tbl_performance_bonus MA 
        LEFT JOIN search_field_emp SFE ON SFE.content_id=MA.content_id 
        ")->result();
        $this->load->view('payroll/performance_bonus/manage_performance_bonus', $data);
    }
    function create()
    {
        ...
        echo  $this->load->view('payroll/performance_bonus/add_performance_bonus', $data);
    }
    function save()
    {
        date_default_timezone_set('Asia/Dhaka');
        $data['title'] = "Save Performance Bonus";
        ...
        if ($editId = $input['edit_id']) {
            $this->db->where('id', $editId)->update('tbl_performance_bonus', $salaryData);
            $this->session->set_flashdata('message', "Performance bonus data updated successfully!");
        } else {
            $this->db->insert('tbl_performance_bonus', $salaryData);
            $this->session->set_flashdata('message', "Performance bonus added successfully!");
        }
    ...
    function check_duplicate($emp_content_id)
    {
        ...
        $result = $this->db->get('tbl_performance_bonus')->num_rows();
