...
class SpecialBonusController extends CI_Controller
{
    ...
    function create()
    {
        ...
        echo  $this->load->view('payroll/special_bonus/add_special_bonus', $data);
    }
    function save()
    {
        date_default_timezone_set('Asia/Dhaka');
        $data['title'] = "Save Special Bonus";
        ...
        if ($editId = $input['edit_id']) {
            $this->db->where('id', $editId)->update('tbl_special_bonus', $salaryData);
            $this->session->set_flashdata('message', "Special bonus data updated successfully!");
        } else {
            $this->db->insert('tbl_special_bonus', $salaryData);
            $this->session->set_flashdata('message', "Special bonus added successfully!");
        }
    ...
    function check_duplicate($emp_content_id)
    {
        ...
        $result = $this->db->get('tbl_special_bonus')->num_rows();
